...
function changeLangage(langue,init) { // Mise en place des textes en fonction de la langue choisie
	...
	$("#aide_kiosque").html('1. '+FS.main.langage[16][langue]+'<br /><br />2. '+FS.main.langage[17][langue]+'<br /><br />3. '+FS.main.langage[18][langue]+'<br /><br />4. '+FS.main.langage[19][langue]+'<br /><br />5. '+FS.main.langage[20][langue]) ;
...
function Superpose() { // mise en place du mode de superposition des images
...
function Switch(swipeURL) { // mise en place du mode avec réglette
...
function HauteurCarte() { // définit les hauteurs de carte en fonction des navigateurs
...
function LargeurCarte() { // définit les hauteurs de carte en fonction des navigateurs
	...
	if(adresse.length>2) {
		...
		$.getJSON(url, function(data) {
			...
			for (var i = 0; i<data.features.length ; i++) {
				...
				$(document).on('click',"#f"+i,{geometrie: data.features[i].geometry},zoomAdresse);
...
function zoomAdresse(e) { // zoom sur les coordonnées du point adresse identifié
...
function Metadonnees(choixImage) { // retourne le bon lien de métadonnées en fonction du choix de couche	
	for(var i in FS.main.donnees) {
...
function ChoixImage() { // gère le changement de couche dans les menus de sélection
...
function ChangeImage(cote) {
	...
	if (chargementCap != 1) {	// chargement des capabilities
		$.when( 				
			...
			if (cote != 'D') {
				lireCapabilities('metadonnees',choixImageGauche,'G',FS.main.donnees[1].length-1);
			};
			if (cote != 'G') {
				lireCapabilities('metadonnees',choixImageDroite,'D',FS.main.donnees[1].length-1);
			};		
...
function afficheAttribution(attributions,cote,couche) {
	...
	if (attributions[1] !== null) {
		$("#logoAttribution"+cote).html('<img src="'+attributions[1]+'" draggable="false" height="50"/>'); //modifié
...
function ChoixOpacite() { // gère le changement d'opacité (utiliser avec le mode superposition)
